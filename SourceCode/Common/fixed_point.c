...
void COPY_NEGATE_FX_ASM(pfx x, pfx y)
{
  x; /* Avoid warning about unused argument, doesn't add any opcodes. */
  y;
  __asm
...
void ADD_FX_ASM(pfx x, pfx y, pfx z)
{
  ...
  ld    a,(bc)    /* Do the 24 bit addition, x + y = z in effect. */
  add   a,(hl)
...
void SUBTRACT_FX_ASM(pfx x, pfx y, pfx z)
{
  x; /* Just avoid warning about unused argument, doesn't add any opcodes. */
  y;
  z;
  ...
  ld    a,(bc)    /* Do the 24 bit subtraction, x - y = z in effect. */
  sub   a,(hl)
  ld    (de),a    /* Save result in z, can overwrite x or y, but that's OK. */
  inc   hl
  inc   bc
...
/* Compare two values X & Y, return a small integer (so it can be returned in
   ...
   +1 if X > Y. */
int8_t COMPARE_FX_ASM(pfx x, pfx y)
{
  x; /* Just avoid warning about unused argument, doesn't add any opcodes. */
  y;
  __asm
  ...
  sub   a,(hl)
  ld    e,a       /* Collect bits to see if whole result is zero, first in E. */
  inc   hl
  inc   bc
  ld    a,(bc)
  sbc   a,(hl)
  ld    d,a       /* Save another result byte for zero detection in D. */
  inc   hl
  inc   bc
  ld    a,(bc)
  sbc   a,(hl)
  ld    h,a       /* Save final byte in H, not using hl any more by here. */
...
NoOverflowCompare:
  ...
  ld    l,0xFF    /* Negative result, so X < Y, return -1. */
  ret
PositiveCompare:
  ld    a,h       /* See if the complete result is zero. */
  or    a,d
  or    a,e
  jr    z,ZeroCompare
  ld    l,0x01    /* Positive result, return +1. */
  ret
ZeroCompare:
  __endasm;
...
#ifdef NABU_H
/* Compare value X against zero and return a small integer which
   is -1 if X < 0, zero if X == 0, +1 if X > 0. */
int8_t TEST_FX_ASM(pfx x)
{
  x; /* Just avoid warning about unused argument, doesn't add any opcodes. */
  __asm
  ld    hl,2      /* Hop over return address. */
  ...
  inc   hl
  ld    h,(hl)
  ld    l,a       /* hl now has pointer to x's fx data, a 24 bit integer. */
  ...
  or    a,a
  jp    P,PositiveTest
  ld    l,0xFF    /* Negative result, so X < 0, return -1. */
  ret
PositiveTest:
  ld    l,0x01    /* Positive, so X > 0, return +1. */
  ret
ZeroTest:
  __endasm;
  return 0;       /* Need at least one return in C, else get warning. */
